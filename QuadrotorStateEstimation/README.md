...
## EKF Flow In This Project ##

This estimator is slightly different from the most compact textbook EKF presentation because the attitude handling is split across multiple functions.

The important design choice is:

- `roll` and `pitch` are estimated in `UpdateFromIMU()`
- the EKF state itself contains:
  - position
  - velocity
  - yaw

So the full estimator cycle is distributed across the following functions.

### 1. IMU Attitude Step: `UpdateFromIMU()`

Purpose:

- use gyroscope rates to predict attitude
- use accelerometer to correct roll and pitch drift

What happens:

1. Build the current attitude from `rollEst`, `pitchEst`, and `ekfState(6)` for yaw.
2. Integrate body angular rates from the gyro forward by `dtIMU`.
3. Extract predicted roll, pitch, yaw from the integrated attitude.
4. Compute `accelRoll` and `accelPitch` from the accelerometer gravity direction.
5. Blend gyro prediction and accelerometer tilt using a complementary filter.
6. Store the gyro-predicted yaw back into `ekfState(6)`.

Important note:

- accelerometer corrects roll and pitch
- accelerometer does not observe yaw
- yaw is later corrected by the magnetometer

### 2. State Mean Prediction: `PredictState()`

Purpose:

- propagate the EKF state mean forward using the motion model
...
1. Build the current attitude quaternion from roll, pitch, and yaw.
2. Rotate IMU acceleration from body frame into global frame:
   - `a_global = Rbg * a_body`
3. Predict position with constant-acceleration integration:
   - `p_new = p + v * dt + 0.5 * a_global * dt^2`
4. Predict velocity:
   - `v_new = v + a_global * dt`
5. Keep yaw unchanged here because yaw integration already happened in `UpdateFromIMU()`.

### 3. Covariance Prediction: `Predict()`

Purpose:

- propagate EKF covariance forward using the local linearization of the motion model

What happens:

1. Call `PredictState()` to get the predicted mean state.
2. Build the process Jacobian `gPrime`.
3. Use `GetRbgPrime()` to compute the partial derivative of the body-to-global rotation matrix with respect to yaw.
4. Use that derivative to calculate how global acceleration changes when yaw changes:
   - `d(a_global)/dyaw = (dRbg/dyaw) * a_body`
5. Fill the Jacobian entries that capture:
   - position dependence on velocity
   - position dependence on yaw through rotated acceleration
   - velocity dependence on yaw through rotated acceleration
6. Propagate covariance:
   - `P_new = gPrime * P * gPrime.transpose() + Q`
7. Commit the predicted state.

Why the Jacobian matters:

- the state mean is propagated by the nonlinear motion model
- the covariance is propagated by a first-order local linearization of that nonlinear model

### 4. GPS Measurement Update: `UpdateFromGPS()`

Purpose:

- correct position and velocity states using GPS

Measurement:

- GPS provides `[x, y, z, vx, vy, vz]`

What happens:

1. Form the measurement vector `z` from GPS position and velocity.
2. Build `zFromX` from the current EKF state:
   - predicted GPS reading from current estimate
3. Build the measurement Jacobian `hPrime`.
   - For GPS this is a selector matrix with ones on position and velocity state entries.
4. Call the generic `Update()` function.

### 5. Magnetometer Measurement Update: `UpdateFromMag()`

Purpose:

- correct yaw using magnetometer yaw

Measurement:

- magnetometer provides yaw

What happens:

1. Form scalar measurement `z = [magYaw]`.
2. Form predicted measurement `zFromX = [ekfState(6)]`.
3. Build `hPrime` so that only yaw is observed:
   - `hPrime(0, 6) = 1`
4. Normalize the yaw innovation so it lies in `[-pi, pi]`.
5. Call the generic `Update()` function.

Why normalization matters:

- angles wrap around
- a yaw residual near the `-pi/pi` boundary must be treated as a small angular difference, not a large one

### 6. Generic EKF Measurement Correction: `Update()`

This function performs the standard EKF update step.
...
- `zFromX`: predicted measurement from current state

What happens:

1. Compute innovation covariance:
   - `S = H * P * H.transpose() + R`
...
This is the reusable EKF correction block used by both GPS and magnetometer updates.

### Summary Of The Full Cycle

At a high level the estimator runs like this:

1. `UpdateFromIMU()`
   - update roll, pitch, and gyro-predicted yaw
2. `PredictState()`
   - propagate the EKF mean state
3. `Predict()`
   - propagate covariance using the Jacobian
4. `UpdateFromGPS()`
   - correct position and velocity when GPS is available
5. `UpdateFromMag()`
   - correct yaw when magnetometer is available
