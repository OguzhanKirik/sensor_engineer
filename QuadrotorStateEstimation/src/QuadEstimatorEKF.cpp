...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  Quaternion<float> predictedAttitude = attitude.IntegrateBodyRate(gyro, dtIMU);

  float predictedRoll = predictedAttitude.Roll();
  float predictedPitch = predictedAttitude.Pitch();
  float predictedYaw = predictedAttitude.Yaw();
  ...
  accelPitch = atan2f(-accel.x, sqrtf(accel.y * accel.y + accel.z * accel.z));

  // FUSE INTEGRATION AND UPDATE
  const float alpha = attitudeTau / (attitudeTau + dtIMU); //altitudeTau, how much you trust the gyro
  rollEst = alpha * predictedRoll + (1.f - alpha) * accelRoll; // alpha how much yu rely on the accelerometer
  pitchEst = alpha * predictedPitch + (1.f - alpha) * accelPitch;
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  V3F accelInGlobal = attitude.Rotate_BtoI(accel);
  accelG = accelInGlobal;
  /*  
  pos_new = pos + vel * dt + 0.5 * accel_global * dt^2
  vel_new = vel + accel_global * dt
  yaw_new = curState(6)
  */
  ...
  predictedState(3) = curState(3) + accelInGlobal.x * dt;
  predictedState(4) = curState(4) + accelInGlobal.y * dt;
  predictedState(5) = curState(5) + accelInGlobal.z * dt;
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  V3F dAccelGlobal_dYaw = V3F(
    RbgPrime(0,0)*accel.x + RbgPrime(0,1)*accel.y + RbgPrime(0,2)*accel.z,
    RbgPrime(1,0)*accel.x + RbgPrime(1,1)*accel.y + RbgPrime(1,2)*accel.z,
    RbgPrime(2,0)*accel.x + RbgPrime(2,1)*accel.y + RbgPrime(2,2)*accel.z
  );
  ...
  gPrime(3,6) = dAccelGlobal_dYaw.x * dt;
  gPrime(4,6) = dAccelGlobal_dYaw.y * dt;
  gPrime(5,6) = dAccelGlobal_dYaw.z * dt;
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  float innovation = magYaw - zFromX(0);
  if (innovation > F_PI) innovation -= 2.f * F_PI;
  if (innovation < -F_PI) innovation += 2.f * F_PI;

  z(0) = zFromX(0) + innovation;
